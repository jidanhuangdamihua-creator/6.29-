...
class UnifiedD1D6OutputContractTest(unittest.TestCase):
    def test_strict_d1_d3_plan_has_all_six_methods_in_each_mode(self):
        cfg = run_full_paper_experiments._load_config()
        protocol = run_full_paper_experiments.load_paper_protocol(cfg)

        plan = run_full_paper_experiments._build_run_plan(
            run_full_paper_experiments.METHODS,
            protocol=protocol,
            strict_paper_mode=True,
        )

        for scenario in run_full_paper_experiments.INFO_SHARING_SCENARIOS:
            methods = [method for method, _, mode, _ in plan if mode == scenario]
            self.assertEqual(list(FORMAL_METHODS), methods)
    ...
    def test_unified_runner_passes_isolated_output_dirs_to_d1_and_d4_tasks(self):
        ...
        for task in tasks:
            ...
            self.assertEqual(
                str(task.expected_result_path.parent.parent),
    ...
    def test_unified_runner_exits_nonzero_when_child_task_fails(self):
        ...
        with patch.object(run_unified_d1_d6, "_parse_args") as parse_args, patch.object(
            ...
            return_value=run_unified_d1_d6.CodeIdentity("abc", False, "0" * 64),
        ), patch.object(
            run_unified_d1_d6, "write_or_validate_run_plan"
        ), patch.object(
            run_unified_d1_d6, "discover_formal_input_identity", return_value={}
        ), patch.object(
            run_unified_d1_d6,
            "_resolve_run_root",
    ...
    def test_unified_runner_rejects_smoke_before_creating_a_run(self):
        with patch.object(
            run_unified_d1_d6,
            "_parse_args",
            return_value=argparse.Namespace(smoke=True),
        ), patch.object(run_unified_d1_d6, "_resolve_run_root") as resolve_root:
            with self.assertRaisesRegex(SystemExit, "--smoke is disabled"):
                run_unified_d1_d6.main()
