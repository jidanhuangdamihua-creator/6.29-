...
from paper_reproduction_protocol import MULTI_SOURCE_TL_METHODS, get_paper_source_counts, load_paper_protocol
...
def _validate_matrix_source_counts(
    source_counts: Sequence[int],
    enabled_methods_options: Sequence[Sequence[str]],
    config: Optional[Dict[str, Any]],
    strict_paper_mode: Optional[bool],
) -> None:
    if not strict_paper_mode:
        return

    protocol = load_paper_protocol(config)
    allowed_counts = set(get_paper_source_counts(protocol))

    all_methods = set()
    for group in enabled_methods_options:
        all_methods.update([str(m) for m in group])

    uses_multi_source = any(method in MULTI_SOURCE_TL_METHODS for method in all_methods)
    if not uses_multi_source:
        return

    invalid_counts = sorted([int(k) for k in source_counts if int(k) not in allowed_counts])
    if invalid_counts:
        raise ValueError(
            "strict paper mode forbids matrix source_counts outside paper protocol. "
            f"invalid={invalid_counts} allowed={sorted(allowed_counts)}"
        )
...
def build_experiment_matrix(
    ...
    for dataset_name in dataset_names:
        for horizon in horizons:
            for number_of_sources in source_counts:
...
def make_experiment_id(experiment_config: Dict[str, Any]) -> str:
    ...
    k_token = f"k{int(experiment_config.get('number_of_sources', experiment_config.get('k', 1)))}"
...
def run_single_experiment_config(
    ...
    experiment_results = run_all_experiments(
        ...
        k=int(experiment_config.get("number_of_sources", experiment_config["k"])),
        number_of_sources=int(experiment_config.get("number_of_sources", experiment_config["k"])),
    ...
    results_df["k"] = int(experiment_config.get("number_of_sources", experiment_config["k"]))
    results_df["number_of_sources"] = int(experiment_config.get("number_of_sources", experiment_config["k"]))
...
def run_experiment_matrix(
    ...
    _validate_matrix_source_counts(
        source_counts=source_counts,
        enabled_methods_options=enabled_methods_options,
        config=config,
        strict_paper_mode=strict_paper_mode,
