...
def accept_mode_matrix(
    ...
    *,
    ...
    if expected.scope is not AcceptanceScope.MODE_MATRIX:
        reasons.append("scope_mismatch")
    expected_pairs = {
        (horizon, seed) for horizon in expected.horizons for seed in expected.seeds
    }
    ...
    for path in cell_paths:
        ...
        discovered[pair] = Path(path)
    if set(discovered) != expected_pairs or len(cell_paths) != len(expected_pairs):
    ...
    if not reasons:
        for pair in sorted(discovered):
            ...
            outcome = accept_cell_csv(discovered[pair], expected=cell_expected)
            if not outcome.report.passed:
