...
def _standardize_rossmann_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "region_id" not in renamed.columns:
        # 注意：当前 full paper solidified-parquet 路径不执行这里的 region_id 占位逻辑。
        # D3 的 no-sharing region 语义若启用，已在 scripts/fix_d3_parquet.py
        # 中通过 ((entity_id - 1) // 10) + 1 固化为 parquet 的 region 列。
        # 不要在这里补造或修改 region_id，避免影响旧动态预处理路径。
...
def is_identifier_like_feature_column(column: str) -> bool:
    """Return True for identifier columns that must not enter model inputs."""
    name = str(column).strip().lower()
    if not name:
        return False
    if name in _SOURCE_SELECTION_EXCLUDE_EXACT:
        return True
...
def infer_source_selection_feature_columns(
    ...
    resolved_mode = str(knn_feature_mode or "paper_available_features_no_ids").strip().lower()
    ...
    return {
        ...
        "knn_feature_mode": resolved_mode,
    }
...
def normalize_features(
    ...
    Args:
        ...
        val_df: 验证集
        test_df: 测试集
        feature_cols: 可选的显式特征列；为空时沿用自动推断
    ...
    if feature_cols is None:
        ...
        feature_columns = list(dict.fromkeys(str(c) for c in feature_cols))
        missing_by_split = {
            "train": [c for c in feature_columns if c not in train_df.columns],
            "val": [c for c in feature_columns if c not in val_df.columns],
            "test": [c for c in feature_columns if c not in test_df.columns],
        }
        missing_by_split = {split: cols for split, cols in missing_by_split.items() if cols}
        if missing_by_split:
            raise ValueError(f"Missing feature_cols in normalize_features: {missing_by_split}")
        unsafe = [c for c in feature_columns if is_identifier_like_feature_column(c)]
        if unsafe:
            raise ValueError(f"ID feature_cols are not allowed in normalize_features: {unsafe}")
        non_numeric = [
            c
        ...
        if non_numeric:
            dtype_map = {c: str(train_df[c].dtype) for c in non_numeric}
            raise ValueError(f"Non-numeric feature_cols in normalize_features: {dtype_map}")
...
def build_tabular_sequence(
    ...
    Args:
        ...
        horizon: 预测步长
        window_size: 历史窗口长度
        feature_cols: 可选的显式 X 特征列；为空时沿用自动推断
    ...
    if feature_cols is None:
        ...
        feature_columns = list(dict.fromkeys(str(c) for c in feature_cols))
        missing = [c for c in feature_columns if c not in ordered.columns]
        if missing:
            raise ValueError(f"Missing feature_cols in build_tabular_sequence: {missing}")
        unsafe = [c for c in feature_columns if is_identifier_like_feature_column(c)]
        if unsafe:
            raise ValueError(f"ID feature_cols are not allowed in build_tabular_sequence: {unsafe}")
        non_numeric = [c for c in feature_columns if not pd.api.types.is_numeric_dtype(ordered[c])]
        if non_numeric:
            dtype_map = {c: str(ordered[c].dtype) for c in non_numeric}
            raise ValueError(f"Non-numeric feature_cols in build_tabular_sequence: {dtype_map}")
    if "sales" not in ordered.columns:
